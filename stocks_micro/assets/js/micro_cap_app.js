...
function xScale(stockData, chosenXAxis) {
  ...
  if (chosenXAxis === "Stock_Price") {
    ...
    .domain([d3.min(stockData, d => d[chosenXAxis]) * 0.9,
      d3.max(stockData, d => d[chosenXAxis]) * 0.3
    ])
    .range([0, width]);
  }
    ...
    // ])
    // .range([0, width]);
    var xLinearScale = d3.scaleLinear().domain([-1.6, 0.9])
    .range([0, width]);
  }
...
d3.csv("./assets/data/micro-cap.csv").then(function(stockData, err) {
  if (err) throw err;
