...
// create scales per choxen x axis
  ...
  if (chosenXAxis === "Stock_Price") {
    ...
    .domain([d3.min(stockData, d => d[chosenXAxis]) * -0.2,
      d3.max(stockData, d => d[chosenXAxis]) * 0.5
    ])
    .range([0, width]);
  }
    ...
    // ])
    // .range([0, width]);
    var xLinearScale = d3.scaleLinear().domain([-1.5, 2.9])
    .range([0, width]);
  }
...
d3.csv("./assets/data/mid-cap.csv").then(function(stockData, err) {
  if (err) throw err;
